...
export default function DevocionalPage() {
  ...
  async function prepare(event?: FormEvent) {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Não foi possível preparar o devocional.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <ModuleShell title="Devocional" description="Um espaço para parar, compreender a Palavra e responder a Deus com reverência e prática.">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        {devotional ? (
          <section className="rounded-[26px] border border-[#E1E3E2] bg-white p-6 shadow-sm md:p-9">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#8C183F]">Devocional preparado</p>
                <p className="mt-1 text-xs text-[#7A8794]">{devotional.anchor_reference ?? devotional.reference_or_theme ?? query}</p>
              </div>
              <span className="rounded-full bg-[#F6F5F2] px-3 py-1.5 text-[10px] font-bold text-[#5E6E82]">{devotional.cached ? "Do acervo" : "Novo"}</span>
            </div>
          ...
          <section className="grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl border border-[#E1E3E2] bg-white p-6"><span className="text-2xl text-[#8C183F]">✦</span><h3 className="mt-4 font-serif text-xl font-bold">Entenda</h3><p className="mt-2 text-sm leading-6 text-[#5E6E82]">Comece pelo que o texto realmente diz.</p></div>
            <div className="rounded-2xl border border-[#E1E3E2] bg-white p-6"><span className="text-2xl text-[#C4A47C]">◌</span><h3 className="mt-4 font-serif text-xl font-bold">Medite</h3><p className="mt-2 text-sm leading-6 text-[#5E6E82]">Deixe a verdade confrontar e iluminar o coração.</p></div>
            <div className="rounded-2xl border border-[#E1E3E2] bg-white p-6"><span className="text-2xl text-[#315494]">→</span><h3 className="mt-4 font-serif text-xl font-bold">Pratique</h3><p className="mt-2 text-sm leading-6 text-[#5E6E82]">Transforme compreensão em uma resposta concreta.</p></div>
          </section>
        )}
      </div>
    </ModuleShell>
