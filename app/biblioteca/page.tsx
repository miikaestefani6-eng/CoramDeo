...
export default function BibliotecaPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const response = await fetch(ASSETS_FUNCTION, {
  ...
  const filtered = useMemo(() => {
    ...
    return assets.filter((asset) => {
      ...
      const searchText = [asset.title, asset.description, asset.category, asset.author_name, asset.speaker_or_channel]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  async function openAsset(asset: Asset) {
    ...
    try {
      ...
      const response = await fetch(`${ASSETS_FUNCTION}?id=${encodeURIComponent(asset.id)}`, {
        headers: { Authorization: `Bearer ${session.access_token}` },
        cache: "no-store",
      });
      const payload = await response.json().catch(() => ({}));
  ...
  return (
    <ModuleShell title="Biblioteca" description="Um acervo para aprofundar seu conhecimento da Palavra e sua formação cristã.">
      <section className="rounded-2xl bg-[#0F2131] p-6 text-white md:p-8">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-white/65">Materiais publicados pela plataforma aparecem aqui automaticamente.</p>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {categories.map((item) => (
          <button key={item} onClick={() => setCategory(item)} className={`rounded-lg px-4 py-2 text-xs font-semibold ${category === item ? "bg-[#8C183F] text-white" : "border border-[#E1E7EA] bg-white text-[#5E6E82]"}`}>{item}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((asset) => {
            ...
            return (
              <article key={asset.id} className="overflow-hidden rounded-2xl border border-[#E1E7EA] bg-white shadow-[0_4px_12px_rgba(0,0,0,.06)]">
      ...
      {!loading && !filtered.length && !error && (
        <div className="mt-6 rounded-2xl border border-dashed border-[#D8D0C5] p-10 text-center text-sm text-[#737B84]">Nenhum conteúdo publicado encontrado.</div>
      )}
    </ModuleShell>
  );
}
