...
export default function LoginClient() {
  ...
  return <main className="min-h-screen bg-[#080D13] text-[#F8F2E8]"><div className="mx-auto grid min-h-screen max-w-7xl lg:grid-cols-[.95fr_1.05fr]">
    ...
    <section className="flex items-center justify-center px-5 py-10 sm:px-8"><div className="w-full max-w-md"><div className="mb-8 lg:hidden"><Link href="/site"><Brand /></Link></div><p className="text-[11px] font-bold uppercase tracking-[.25em] text-[#D5B579]">{isReset ? "Redefinir senha" : mode === "login" ? "Bem-vindo de volta" : "Comece sua jornada"}</p><h2 className="mt-3 font-serif text-4xl font-semibold text-[#F8F2E8]">{isReset ? "Crie uma nova senha." : mode === "login" ? "Entre no Coram Deo." : "Crie seu acesso."}</h2><p className="mt-3 text-sm leading-6 text-white/42">{isReset ? "Use uma nova senha segura para continuar." : mode === "login" ? "Continue de onde você parou." : "Sua jornada de estudo começa com um acesso simples e seguro."}</p>
