...
export default function AdminConteudosPage() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase.from("coram_content_assets").select("id,kind,title,description,external_url,storage_path,cover_storage_path,author_name,category,status,access_tier,is_featured,sort_order").order("sort_order").order("created_at", { ascending: false });
  ...
  function edit(asset: Asset) {
    ...
    setForm({ id: asset.id, kind: asset.kind, title: asset.title, description: asset.description || "", external_url: asset.external_url || "", storage_path: asset.storage_path || "", cover_storage_path: asset.cover_storage_path || "", author_name: asset.author_name || "", category: asset.category || "", status: asset.status, access_tier: asset.access_tier, is_featured: asset.is_featured, sort_order: asset.sort_order });
  ...
  async function save(event: FormEvent) {
    ...
    let coverStoragePath = form.cover_storage_path || null;
    ...
    if (file) {
      ...
      uploadedContentPath = storagePath;
    }

    if (coverFile) {
      const allowed = ["image/jpeg", "image/png", "image/webp"];
      if (!allowed.includes(coverFile.type)) {
        if (uploadedContentPath) await supabase.storage.from("coram-content").remove([uploadedContentPath]);
        setError("A capa deve ser uma imagem JPG, PNG ou WebP."); setSaving(false); return;
      }
      if (coverFile.size > 5 * 1024 * 1024) {
        if (uploadedContentPath) await supabase.storage.from("coram-content").remove([uploadedContentPath]);
        setError("A imagem de capa deve ter no máximo 5 MB."); setSaving(false); return;
      }
      const safeCoverName = coverFile.name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9._-]/g, "-");
      coverStoragePath = `${user.id}/covers/${crypto.randomUUID()}-${safeCoverName}`;
      const { error: coverUploadError } = await supabase.storage.from("coram-content").upload(coverStoragePath, coverFile, { contentType: coverFile.type, upsert: false });
      if (coverUploadError) {
        if (uploadedContentPath) await supabase.storage.from("coram-content").remove([uploadedContentPath]);
        setError(coverUploadError.message || "Não foi possível enviar a capa."); setSaving(false); return;
      }
      uploadedCoverPath = coverStoragePath;
    }

    const externalUrl = file ? null : (form.external_url.trim() || null);
    const payload = { kind: form.kind, title: form.title.trim(), description: form.description.trim() || null, external_url: externalUrl, storage_path: storagePath, cover_storage_path: coverStoragePath, source_type: storagePath ? "upload" : externalUrl ? "external" : "upload", author_name: form.author_name.trim() || null, category: form.category.trim() || null, status: form.status, access_tier: form.access_tier, is_featured: form.is_featured, sort_order: Number(form.sort_order) || 100, updated_at: new Date().toISOString(), ...(form.id ? {} : { created_by: user.id }), ...(form.status === "published" ? { published_at: new Date().toISOString() } : {}) };
    ...
    if (result.error) {
      const cleanup = [uploadedContentPath, uploadedCoverPath].filter(Boolean) as string[];
      if (cleanup.length) await supabase.storage.from("coram-content").remove(cleanup);
      setError(result.error.message || "Não foi possível salvar o conteúdo.");
    } else {
      if (form.id && coverFile && form.cover_storage_path && form.cover_storage_path !== coverStoragePath) await supabase.storage.from("coram-content").remove([form.cover_storage_path]);
      setMessage(form.id ? "Conteúdo atualizado." : "Conteúdo criado e disponível no acervo conforme o status escolhido.");
      setForm(emptyForm); setFile(null); setCoverFile(null); await load();
    }
    setSaving(false);
  }
