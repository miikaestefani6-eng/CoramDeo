...
export default function AdminConteudosPage() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      ...
      .select("id,kind,title,description,external_url,storage_path,author_name,category,status,access_tier,is_featured,sort_order")
      .order("sort_order")
  ...
  async function save(event: FormEvent) {
    ...
    const payload = {
      ...
      source_type: storagePath ? "upload" : externalUrl ? "external" : "upload",
    ...
    if (result.error) {
      if (file && storagePath) await supabase.storage.from("coram-content").remove([storagePath]);
      ...
      setForm(emptyForm); setFile(null); await load();
    }
    setSaving(false);
  }
  ...
  async function remove(id: string) {
    ...
    if (error) setError("Não foi possível excluir o conteúdo.");
    else {
      if (asset?.storage_path) await supabase.storage.from("coram-content").remove([asset.storage_path]);
      await load();
    }
  }
  ...
  return <main className="min-h-screen bg-[#F6F5F2] px-5 py-8 text-[#0F2131] lg:px-10">
    <div className="mx-auto max-w-6xl space-y-7">
      ...
      <form onSubmit={save} className="grid gap-4 rounded-2xl border bg-white p-6 shadow-sm md:grid-cols-2">
        ...
        <div className="md:col-span-2 rounded-xl border border-dashed p-4"><label className="text-sm font-semibold">Arquivo PDF / E-book</label><input type="file" accept="application/pdf,.pdf" onChange={e=>setFile(e.target.files?.[0] || null)} className="mt-2 block w-full text-sm"/><p className="mt-2 text-xs text-[#6A7885]">Até 50 MB. O arquivo fica privado e é entregue ao aluno por link temporário seguro.{form.storage_path && !file ? " Este conteúdo já possui um arquivo enviado." : ""}</p></div>
        <input value={form.external_url} onChange={e=>setForm({...form,external_url:e.target.value})} disabled={!!file} className="rounded-xl border px-4 py-3 md:col-span-2 disabled:bg-slate-50 disabled:text-slate-400" placeholder="Ou use uma URL externa (opcional)" />
